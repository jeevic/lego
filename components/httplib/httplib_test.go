...
func TestHLRequest_PostFileBytes(t *testing.T) {
	fh, _ := os.Open("./README.md")
	b, _ := ioutil.ReadAll(fh)

	h := Post("http://www.baidu.com").PostFileBytes("pic", "picfile", b).Debug(true)
	h.PostFile("pic1", "./README.md")

	fh1, _ := os.Open("./README.md")
	ra := ioutil.NopCloser(fh1)
	h.PostFileReader("pic2", "picfile2", ra)
	res, _ := h.Response()
	d := h.DumpRequest()
