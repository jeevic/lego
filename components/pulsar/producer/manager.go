...
func Register(instance string, setting *setting) error {
	...
	if _, ok := mg.instances[instance]; ok {
		return errors.New(fmt.Sprintf("pulsar producer instance:%s has exists!", instance))
	}
	cf, err := NewProducer(setting)
	if err != nil {
		return err
	}
...
func GetProducer(instance string) (*Producer, error) {
	if ins, ok := mg.instances[instance]; ok {
		...
		return nil, errors.New(fmt.Sprintf("pulsar producer instance:%s not exists!", instance))
	}
}
func Reset() {
	defer mg.mutex.Unlock()
